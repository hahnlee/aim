...
bool BuildSystemClassPath(const char* image_root, const char* support_dex,
  ...
  if (result == nullptr || !valid(image_root) || !valid(support_dex)) return false;
...
int Run(JNIEnv* env, const char* socket_path, InstalledRecordResolver resolver) {
  ...
  // This service-directory process is not an ActivityThread process and must
  // prepare the Looper used by its Android service endpoints itself.
