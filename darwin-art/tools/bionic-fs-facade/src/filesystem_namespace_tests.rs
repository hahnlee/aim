...
fn installed_packages_mount_read_only_at_data_app_under_private_data() {
    ...
    assert_eq!(
        ...
        fs::metadata(f.0.join("packages/example/1/sha/base.apk"))
            .unwrap()
            .ino()
    );
    assert!(
        guest
            .open(b"/data/app/example/1/sha")
            .unwrap()
            .node
            .metadata()
            .is_dir()
    ...
    assert!(
        FilesystemNamespace::with_storage(
            f.root(),
            b"/system",
            b"/system",
            None,
