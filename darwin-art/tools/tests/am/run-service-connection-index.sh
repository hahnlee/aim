...
out="$(mktemp -d "${TMPDIR:-/tmp}/service-connection-index.XXXXXX")"
trap 'rm -rf -- "$out"' EXIT

"$java_home/bin/javac" --release 8 -encoding UTF-8 -cp "$android_jar" -d "$out" \
  ...
  "$root/tools/tests/am/ServiceConnectionIndexTest.java"
"$java_home/bin/java" -ea -cp "$out:$android_jar" \
  dev.darwinart.runtime.am.ServiceConnectionIndexTest
