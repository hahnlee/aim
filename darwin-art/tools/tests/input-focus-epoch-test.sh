...
out=$(mktemp "${TMPDIR:-/tmp}/darwin-art-input-focus-epoch.XXXXXX")
trap 'rm -f "$out"' EXIT
cxx=${CXX:-clang++}
sdk=$(xcrun --sdk macosx --show-sdk-path)
set -- \
  "$root/tools/tests/input-focus-epoch-test.cc" \
...
if [ -f "$root/runtime/framework/input/input_window_state.cc" ]; then
  set -- "$@" "$root/runtime/framework/input/input_window_state.cc"
fi
  set -- "$@" \
...
"$cxx" -arch arm64 -isysroot "$sdk" -std=c++20 -ObjC++ -fobjc-arc -O1 -g -Wall -Wextra -Werror \
