...
stage="$(mktemp -d "${TMPDIR:-/tmp}/image-consumer-queue.XXXXXX")"
...
xcrun clang++ -std=c++20 -Wall -Wextra -Werror -Wno-nullability-completeness \
  ...
  "$root/tools/tests/image-consumer-queue-test.cc" \
  "$root/compat/media/consumer_buffer.cc" \
  "$root/compat/media/image_consumer_queue.cc" -o "$stage/test"
"$stage/test"
