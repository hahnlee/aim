...
build_dir=$(mktemp -d "${TMPDIR:-/tmp}/darwin-art-surface-control-registry.XXXXXX")
...
xcrun clang++ -arch arm64 -isysroot "$sdk" -std=c++20 -Wall -Wextra -Werror \
  ...
  "$repo_dir/tools/tests/surface-control-registry-test.cc" \
  -o "$build_dir/surface-control-registry-test"

"$build_dir/surface-control-registry-test"
