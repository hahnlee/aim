...
| Fullscreen / backing scale | The green button and View > Enter Full Screen (Cmd+Ctrl+F) take the root to macOS fullscreen; its resize is a task revision at the full display extent and exit restores the previous one (Blue Archive 1280×720 → 3456×2168 → 1280×720, clicks mapped). Each task keeps its own raster scale: a root on a display of another backing scale (window moved, or registration) gets one revision changing density (160 × scale) and pixels together. The root's macOS display names Android's display (name, internal/external, physical DPI from CoreGraphics) and a screen move is `EVENT_DISPLAY_BASIC_CHANGED`. Immersive requests do not force macOS fullscreen (desktop convention). Not physically verified: no 1× display attached. Vsync is still a fixed 60 Hz timer (#68). |
...
1. Run locale/label checks, then extend Chromium focus/tab/soak coverage (#19, #22).
2. Close relevant WMS/Binder/SCM lifetime gaps (#18, #20) and audit changed
